...
class _CatalogScreenState extends State<CatalogScreen> {
  ...
  Future<void> _loadCatalog() async {
    ...
    try {
      ...
      ErrorHandler.show(context, e, customMessage: 'Не удалось загрузить каталог');
    }
  }
    ...
    setState(() {
      _filteredItems = _allItems.where((item) {
        ...
        final matchesCategory =
            _selectedCategory == 'Все' || item.category == _selectedCategory;
                                ...
                                Expanded(
                                  ...
                                  child: _buildActionCard(
                                    ...
                                    onTap: () => Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (context) => const ClubMapScreen(),
                                      ),
                                    ),
                                  ),
                                ),
  ...
  Widget _buildCatalogItemCard(CatalogItem item) {
    ...
    final isService = cat.contains('computer') ||
        cat.contains('комп') ||
        cat.contains('pc') ||
        cat.contains('пк') ||
        cat.contains('аренда');
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(
                  '${item.price.toStringAsFixed(0)} ₽${isService ? '/ч' : ''}',
                    ...
                    onPressed: () {
                      if (isService) {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => BookingScreen(initialPcName: item.name),
                          ),
                        );
                      } else {
